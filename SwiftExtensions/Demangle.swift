//swiftlint:disable no_fallthrough_only empty_count syntactic_sugar shorthand_operator unused_optional_binding
//
//  CwlDemangle.swift
//  CwlDemangle
//
...
fileprivate extension TextOutputStream {
	mutating func write<S: Sequence, T: Sequence>(sequence: S, labels: T, render: (inout Self, S.Iterator.Element) -> Void) where T.Iterator.Element == String? {
	...
	mutating func write<S: Sequence>(sequence: S, prefix: String? = nil, separator: String? = nil, suffix: String? = nil, render: (inout Self, S.Iterator.Element) -> Void) {
		if let p = prefix {
			write(p)
		}
		var first = true
	...
	mutating func write<T>(optional: Optional<T>, prefix: String? = nil, suffix: String? = nil, render: (inout Self, T) -> Void) {
		if let p = prefix {
			write(p)
		}
		if let e = optional {
	...
	mutating func write<T>(value: T, prefix: String? = nil, suffix: String? = nil, render: (inout Self, T) -> Void) {
		if let p = prefix {
			write(p)
		}
		render(&self, value)
...
private struct _ScalarScanner<C: Collection> where C.Iterator.Element == UnicodeScalar {
